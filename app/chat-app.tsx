...
import {api,enterChat,getSessionToken,saveSessionToken,ApiError,clockTime,type Room,type Message,type Profile} from "./chat-types";
export default function ChatApp({signedIn:initiallySignedIn}:{signedIn:boolean}) {
 ...
 const [search,setSearch]=useState(""),[drafts,setDrafts]=useState<Record<string,string>>({}),[modal,setModal]=useState<"create"|"profile"|"invite"|"guest"|null>(null),[info,setInfo]=useState(false),[mobileChat,setMobileChat]=useState(false),[emoji,setEmoji]=useState(false);
 const [loading,setLoading]=useState(signedIn),[busy,setBusy]=useState(false),[sending,setSending]=useState(false),[error,setError]=useState(""),[toast,setToast]=useState(""),[connected,setConnected]=useState(true),[more,setMore]=useState(false),[historyBusy,setHistoryBusy]=useState(false),[ready,setReady]=useState(false);
 ...
 async function initialize(){
   ...
   try {await api("",{action:"init"});const list=await refreshRooms();const q=new URLSearchParams(location.search),token=q.get("join");
     if(token){const j=await api("",{action:"join",token});await refreshRooms();setSelected(j.id);setMobileChat(true);history.replaceState(null,"",location.pathname);}
     else if(list.length)setSelected(list[0].id);
     setReady(true);
   } catch(e){setError((e as Error).message);if(e instanceof ApiError && e.status===401){saveSessionToken("");setSignedIn(false);setReady(false);setSelected("");setModal("guest");}}finally{setLoading(false);}
 }
 useEffect(()=>{setInfo(window.innerWidth>1200);try{setDrafts(JSON.parse(sessionStorage.getItem("banke-drafts")||"{}"));}catch{} if(initiallySignedIn||getSessionToken()){setSignedIn(true);void initialize();}else if(new URLSearchParams(location.search).has("join")){setModal("guest");}},[]);
 ...
 function inviteUrl(){return location.origin+location.pathname+"?join="+room?.token;}
 ...
 function newRoom(){if(!signedIn){setError("");setModal("guest");return;}setError("");setModal("create");}
 async function submitModal(e:React.FormEvent<HTMLFormElement>){
   ...
   try{if(modal==="guest"){const result=await enterChat(String(form.get("name")));const saved=saveSessionToken(result.token);setSignedIn(true);setModal(null);await initialize();if(!saved)notify("已进入聊天；此浏览器无法保存身份，关闭后需重新加入。 ");return;}if(modal==="create"){const result=await api("",{action:"create",name:String(form.get("name")),description:String(form.get("description"))});await refreshRooms();setSelected(result.id);setMobileChat(true);notify("房间已创建，邀请朋友一起聊聊吧");}
 ...
 return <div className={"app-shell"+(mobileChat?" show-chat":"")}>
  ...
  {modal&&<Modal title={modal==="create"?"新建聊天室":modal==="profile"?"你的聊天昵称":modal==="guest"?"怎么称呼你？":"邀请好友"} onClose={()=>{if(!busy){setModal(null);setError("");}}}>{modal==="invite"?<div className="invite-modal"><p>把下方链接发给朋友，填写昵称即可加入房间。</p><input aria-label="邀请链接" readOnly value={inviteUrl()} onFocus={e=>e.target.select()}/><button className="primary-button" onClick={copyInvite}><Copy size={17}/>复制链接</button></div>:<form onSubmit={submitModal}><p className="modal-description">{modal==="create"?"给这段对话起个名字，邀请想聊的人。":modal==="guest"?"填写昵称即可开始，无需注册账号。":"朋友们会在消息和房间成员中看到这个名字。"}</p><label>{modal==="create"?"房间名称":"昵称"}<input name="name" required maxLength={modal==="create"?32:24} defaultValue={modal==="profile"?profile?.name:""} placeholder={modal==="create"?"例如：日常闲聊": "怎么称呼你？"} autoFocus/></label>{modal==="create"&&<label>房间介绍 <span>选填</span><textarea name="description" maxLength={160} placeholder="聊聊今天的小事，分享生活中的点滴…" rows={3}/></label>}{error&&<p className="form-error" role="alert">{error}</p>}<button className="primary-button modal-submit" disabled={busy}>{busy?<LoaderCircle className="spin" size={17}/>:modal==="create"?<Plus size={17}/>:<Check size={17}/>} {busy?"请稍候…":modal==="create"?"创建聊天室":modal==="guest"?"进入聊天":"保存昵称"}</button>{modal==="guest"&&<p className="guest-note">身份保存在此浏览器中。换设备或清除浏览器数据后，请通过邀请链接重新加入。</p>}</form>}</Modal>}
 </div>;
}
